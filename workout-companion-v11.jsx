...
function WeightScroller({ value, min, max, step, isCardio, lastVal, pb, onChange }) {
  ...
  return (
    <div className="relative" style={{ height: 88 }}>
      <div aria-hidden="true" className="absolute" style={{ left: "50%", top: 18, transform: "translateX(-50%)", width: 3, height: 60, background: C.cobalt, borderRadius: 2, zIndex: 2, pointerEvents: "none", boxShadow: `0 0 0 4px ${C.floor}` }} />
      ...
      <div style={page}>
        ...
        <div className="max-w-md mx-auto px-5 pt-8 pb-10">
          <div className="flex items-center justify-between">
            <p style={{ color: C.dust, fontWeight: 700, letterSpacing: "0.02em", fontSize: 13 }}>Working Set</p>
            <button
              onClick={() => setScreen("history")}
              className="ws-press"
              style={{ background: "none", border: `1px solid ${C.line}`, color: C.chalk, borderRadius: 10, padding: "8px 14px", fontWeight: 700, fontSize: 13, cursor: "pointer" }}
            >
              History
            </button>
          </div>
          <h1 className="mt-4" style={{ fontSize: 34, fontWeight: 800, lineHeight: 1.1, letterSpacing: "-0.02em" }}>
            ...
            {GROUPS.map((g) => {
              ...
              return (
                <button
                  ...
                  className="ws-press text-left px-4 py-4"
                  style={{
                ...
                >
                  ...
                  <span className="block mt-1" style={{ color: C.dust, fontSize: 13 }}>{g.hint}</span>
                </button>
              );
            })}
          </div>
          ...
          <button
            ...
            role="switch"
            aria-checked={restOn}
            className="ws-press mt-8 w-full flex items-center justify-between gap-4 px-4 py-4 text-left"
            style={{ background: C.surface, border: `1px solid ${C.line}`, borderRadius: 14, cursor: "pointer", color: C.chalk }}
          >
            <span>
              <span style={{ fontWeight: 700, fontSize: 16 }}>Rest timer between sets</span>
              <span className="block mt-1" style={{ color: C.dust, fontSize: 13 }}>
                {restOn ? "Starts automatically after each set" : "Off. You can still start one manually mid-workout."}
              </span>
            </span>
            <span className="relative" style={{ width: 52, height: 30, borderRadius: 999, background: restOn ? C.cobalt : C.line, flexShrink: 0, transition: "background .15s ease" }}>
              <span className="absolute" style={{ top: 3, left: restOn ? 25 : 3, width: 24, height: 24, borderRadius: 999, background: C.chalk, transition: "left .15s ease" }} />
            </span>
          </button>

          <div className="mt-8">
            <button className="ws-press" style={btnPrimary(selectedGroups.length > 0)} disabled={!selectedGroups.length} onClick={() => { setPicked([]); setScreen("pick"); }}>
              Choose exercises
            </button>
          </div>
        </div>
  ...
  if (screen === "workout") {
    ...
    return (
      <div style={page}>
        ...
        <div className="max-w-md mx-auto px-5 pt-4 pb-44">
          <div className="flex items-center justify-between">
            <p style={{ color: C.dust, fontSize: 14, fontWeight: 600 }}>
              Exercise {idx + 1} of {plan.length}
            </p>
            <div className="flex items-center gap-3">
              {lifting != null && (
                <span className="tabular-nums" style={{ color: C.dust, fontSize: 14, fontWeight: 700 }}>{fmtClock(lifting)}</span>
              )}
              <button onClick={() => finish(plan)} style={{ background: "none", border: `1px solid ${C.line}`, color: C.dust, borderRadius: 10, padding: "6px 12px", fontWeight: 600, cursor: "pointer", fontSize: 13 }}>
                End
              </button>
            </div>
          </div>

          <p className="mt-3" style={{ color: C.dust, fontSize: 13, fontWeight: 700 }}>{grp.label}</p>
          <div className="mt-1 flex items-center justify-between gap-3">
            <h1 style={{ fontSize: 26, fontWeight: 800, letterSpacing: "-0.02em" }}>{ex.name}</h1>
            <button
              onClick={openChange}
              className="ws-press"
              style={{ background: "none", border: `1px solid ${C.line}`, color: C.chalk, borderRadius: 10, padding: "6px 12px", fontWeight: 700, cursor: "pointer", fontSize: 13, flexShrink: 0 }}
            >
              Change
            </button>
          </div>
          ...
          <div className="mt-2 flex items-center gap-3">
            <div>
              <p style={{ color: C.dust, fontSize: 14 }}>
                {hasHist ? (isCardio ? `Last time ${lastVal} min` : `Last time ${fmtW(lastVal)} x ${ex.reps}`) : "First time, no history yet"}
              </p>
              {ex.pb != null && (
                <p style={{ color: C.amber, fontSize: 14, fontWeight: 700 }}>Personal best {isCardio ? `${ex.pb} min` : fmtW(ex.pb)}</p>
              )}
            </div>
          </div>

          {ex.cues.length > 0 && (
            <div className="mt-3 pl-3" style={{ borderLeft: `2px solid ${C.cobalt}` }}>
              {ex.cues.map((c, i) => (
                <p key={i} style={{ color: C.dust, fontSize: 13, lineHeight: 1.7 }}>{c}</p>
              ))}
            </div>
          )}
          ...
          <div className="mt-5 text-center">
            <p className="tabular-nums" style={{ fontSize: 76, fontWeight: 800, lineHeight: 1, letterSpacing: "-0.03em" }}>
              {!isCardio && weight === 0 ? "BW" : weight}
              {(isCardio || weight !== 0) && (
                <span style={{ fontSize: 22, fontWeight: 700, color: C.dust, marginLeft: 8 }}>{unit}</span>
              )}
            </p>
            {diff != null && (
              <p className="mt-1" style={{ fontSize: 14, fontWeight: 600, color: diff > 0 ? C.sage : C.dust }}>
                {diff === 0 ? "Same as last time" : `${diff > 0 ? "+" : ""}${diff} ${unit} vs last time`}
              </p>
            )}
            {ex.pb != null && weight > ex.pb && (
              <p className="mt-1" style={{ fontSize: 14, fontWeight: 700, color: C.amber }}>
          ...
          <div className="mt-4">
            ...
            />
            {(hasHist || ex.pb != null) && (
              <div className="mt-1 flex items-center justify-center gap-5" style={{ fontSize: 12, fontWeight: 600 }}>
          ...
          <div className="mt-5">
            <button className="ws-press" style={btnPrimary(true)} onClick={logSet}>
          ...
          <div className="mt-6 flex items-center gap-3">
            <button
              ...
              style={{ ...btnQuiet, minHeight: 52, opacity: sets.length ? 1 : 0.5 }}
              onClick={advance}
